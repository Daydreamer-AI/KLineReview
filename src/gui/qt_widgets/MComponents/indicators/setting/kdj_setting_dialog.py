...
class KdjSettingDialog(QDialog):
    def __init__(self, parent=None):
        super(KdjSettingDialog, self).__init__(parent)
        uic.loadUi('./src/gui/qt_widgets/MComponents/indicators/setting/KdjSettingDialog.ui', self)
    ...
    def init_para(self):
        ...
        self.dict_setting_object = {
            0: KdjSettingObject(0, self.lineEdit_ma_period, self.checkBox_ma, self.lineEdit_line_width, self.btn_color),
            1: KdjSettingObject(1, self.lineEdit_ma_period_1, self.checkBox_ma_1, self.lineEdit_line_width_1, self.btn_color_1),
            2: KdjSettingObject(2, self.lineEdit_ma_period_2, self.checkBox_ma_2, self.lineEdit_line_width_2, self.btn_color_2)
        }
    ...
    def slot_btn_apply_clicked(self):
        for id, setting_object in self.dict_setting_object.items():
            if id in self.dict_setting_user.keys():
                ...
                if id == 0:
                    self.dict_setting_user[id].name  = f'{IndicatrosEnum.KDJ_K.value}'
                elif id == 1:
                    self.dict_setting_user[id].name  = f'{IndicatrosEnum.KDJ_D.value}'
                else:
                    self.dict_setting_user[id].name  = f'{IndicatrosEnum.KDJ_J.value}'
