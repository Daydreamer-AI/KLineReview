...
class VolumeWidget(BaseIndicatorWidget):
    ...
    def slot_global_update_labels(self, sender, closest_index):
        ...
        volume = self.df_data.iloc[closest_index]['volume']

        template = self.tr("Volume: %s")
        formatted_volume = QLocale().toString(float(volume), 'f', 2)
        self.label_total_volume.setText(template % formatted_volume)
