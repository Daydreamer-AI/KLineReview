...
2. 补齐 `get_label` / `from_label` / `get_number_label` /
...
sequenceDiagram
    ...
    participant BS as BaostockDataFetchTask
...
| `src/gui/qt_widgets/MComponents/review/review_widget.py` | 复盘加载链路 | `load_data`、`_on_all_periods_loaded`、`_normalize_day_base`、`_start_period_fetch_chain` |
...
| `src/thread/baostock_data_fetch_task.py` | 远程基周期拉取 | `BaostockDataFetchTask.execute` |
