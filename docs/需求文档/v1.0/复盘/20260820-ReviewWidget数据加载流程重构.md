# ReviewWidget 数据加载流程重构

## 基本信息

- 需求编号：TODO（编号规则未定）
- 提出日期：2026-08-20
- 提出人：TODO
- 优先级：P1（重要）
- 状态：开发中
- 关联分支：feature/review-data-loading（从 feature/indicators-data-injection 切出）

## 1. 背景与目标

- 背景：ReviewWidget 目前的数据加载存在多个触发入口（“加载”按钮、“随机加载”按钮、图表内部周期切换按钮），职责混乱；本地无数据时 `BaostockDataFetchTask2` 下载完成后只隐藏 Loading、不回填数据，后台加载链路实际是断的；顶部 `comboBox_period` 仅记录日志，未实际参与加载。
- 目标：复盘数据加载统一由“加载 / 随机加载”按钮触发，一次性在后台加载**所有配置周期**（默认 5/15/30/60 分钟、日线、周线，预留调整接口）的数据，全部加载完成后再统一同步到 `indicators_view_widget`；`comboBox_period` 选择的是“加载完成后初始显示哪个周期”；后台加载期间禁用图表内部周期切换按钮，同步完成后再启用；图表内部周期切换按钮不再触发上层数据加载。

## 2. 需求描述（功能点）

- [x] 统一入口：`load_data(code, date)` 仅由两个按钮触发；移除 `sig_period_changed → 上层取数` 的链路。
- [x] 全周期后台加载：一次加载动作覆盖所有配置周期（默认 5/15/30/60 分、日线、周线）；因 baostock 为全局单会话、并发查询不安全，各周期按顺序串行提交 `BaostockDataFetchTask2`。
- [x] 预留周期列表调整接口：`get_load_periods()` / `set_load_periods(periods)`，调整后同步刷新 `comboBox_period` 选项。
- [x] 周期选择：`comboBox_period` 仅决定加载完成后 `IndicatorsViewWidget` 的初始显示周期；所选周期无数据时回退到第一个可用周期并同步下拉框。
- [x] 异步回填：全部周期加载完成后统一 `set_stock_data(code, {所有周期: df})` → `set_period_buttons_enabled(已加载周期)` → `set_current_period(显示周期)` → `init_animation`，自动出图。
- [x] 加载期间禁用 `IndicatorsViewWidget` 全部周期切换按钮（`set_period_buttons_enabled([])`）；同步完成后仅启用已加载周期的按钮。
- [x] 修复 `BaostockDataFetchTask2` 日线/周线分支恒为 `False` 的 bug（`TimePeriod == TimePeriod.DAY` 为类与枚举成员比较）。
- [x] 图表内部周期按钮：仅在已注入周期数据之间切换；点击未注入周期时回退到原周期并提示，不触发任何上层取数。
- [x] 加载完成后同步图表内部周期按钮（新增 `IndicatorsViewWidget.set_current_period`）。
- [x] 同股票全部配置周期均已加载时重复点击：仅按新日期重新定位动画，不重复下载。
- [x] 本地某周期完全无数据时指标计算会抛 `ValueError("数据为空")`，统一按“无数据”捕获并走后台下载。
- [x] 最新一行查询不依赖后台加载缓存：改用 `get_lastest_row_data_dict_by_code_list` 直接读库，避免“缓存非空但缺该 code”时 `get_lastest_row_data_dict_by_code_list_auto` 抛 KeyError（后台未加载完成时点击加载按钮无问题）。
- [x] 修复 `set_current_period` 误匹配“分时”按钮的问题（`TimePeriod.from_label('分时')` 默认返回日线，导致日线按钮未被选中），加载完成后周期按钮正确处于选中状态。
- [x] 随机加载与代码校验不依赖后台日线缓存：新增 `BaostockDataManager.get_all_stock_code_name_dict()`（从本地股票信息库同步构建 `{code: name}` 映射并缓存），随机选股时直接读库确认该股有日线数据，为后续“个股代码-名称”功能预留 API。

## 3. 验收标准

- [ ] 点击“加载 / 随机加载”后，后台按顺序加载全部配置周期；本地已有周期直接使用，缺失周期自动下载，全部完成后统一出图。
- [ ] `comboBox_period` 选择的周期即加载完成后图表初始显示周期；图表内部周期按钮在已加载周期之间可自由切换。
- [ ] 后台加载期间图表内部周期切换按钮全部禁用；加载完成后仅已加载周期按钮可点击。
- [ ] 日线/周线本地无数据时可正确触发后台下载（原恒 `False` bug 已修复）。
- [ ] 同股票重复点击加载仅按新日期重新定位动画，不触发重复下载。
- [ ] 冒烟验证：从项目根目录运行 `python ./src/main.py`，`data/logs` 无新增 ERROR/异常。

## 4. 影响范围

- 涉及模块/文件：
  - `src/gui/qt_widgets/MComponents/review_widget.py`（本次重构主体）
  - `src/gui/qt_widgets/MComponents/indicators/indicators_view_widget.py`（新增 `set_current_period`、`set_period_buttons_enabled`；复盘模式周期切换守卫；`init_animation` 初始周期状态修正）
  - `src/manager/bao_stock_data_manager.py`（新增 `get_all_stock_code_name_dict`）
  - `src/thread/baostock_data_fetch_task.py`（修复日线/周线分支）
  - `docs/项目导读.md`（5.3 复盘数据流章节同步）
- 涉及数据/数据库：无 schema 变更。
- 兼容性与风险：行情页周期切换仍走 `sig_period_changed → 外部取数`，不受影响；`IndicatorsViewWidget` 为共享组件，改动集中在复盘模式分支与非触发式新增接口；baostock 全局单会话，多周期任务必须串行，不可并行提交。

## 5. 开发记录

- 开发分支：feature/review-data-loading
- 关键提交：TODO（暂未提交）
- 自测结果：py_compile 通过；离屏（QT_QPA_PLATFORM=offscreen）实例化 ReviewWidget 通过；默认周期列表与下拉选项（5/15/30/60 分、日线、周线）正确；`set_period_buttons_enabled` 禁用/按列表启用通过；`set_load_periods` 调整接口刷新下拉通过；以假线程池模拟全周期后台链式加载 → 统一 `set_stock_data` → 按所选周期显示 → 动画初始化通过；同股票重复点击走缓存重定位通过；模拟“后台缓存非空但缺该 code”点击加载不再 KeyError；加载完成后周期按钮正确选中默认周期；后台日线缓存为空时随机选股/代码校验/加载均正常，`get_all_stock_code_name_dict` 同步返回 5182 条代码-名称映射（2026-08-20）。
- 冒烟验证：后台下载链路（本地无数据 → `BaostockDataFetchTask2` → 回填）需真实联网复验，待用户从项目根目录运行 `python ./src/main.py` 确认。

## 6. 完成状态与备注

- 完成日期：TODO（代码已实现，待联网冒烟后置为已完成）
- 遗留问题/TODO：
  - 完全冷启动股票（本地无任何周期数据）若配置周期列表中不含日线，后台下载完成后仍可能取不到“最新一行”（`get_lastest_row_data_dict_by_code_list_auto` 依赖日线表），是否需要自动补拉日线待确认；默认列表含日线，故默认配置下无此问题。
  - `get_random_date` 的 `strptime` 格式疑似有误（`%Y-%M-%D`），本次不修改，另行确认。
  - 本地数据中 5 分钟线缺失时，默认列表会触发 5 分钟线后台下载（`stock_data_5m` 表已支持），需联网复验。
- 备注：本需求为 `20260820-IndicatorsViewWidget数据外部注入重构.md` 的复盘页数据流后续规划。
